...
export function compareVersions(oldActivities: Activity[], newActivities: Activity[]): VersionDiff {
  ...
  for (const a of oldActivities) {
    oldMap.set(a.fingerprint, a);
  }
  for (const a of newActivities) {
    newMap.set(a.fingerprint, a);
  }
  ...
  for (const [fp, newActivity] of newMap) {
    if (!oldMap.has(fp)) {
      ...
      if (changes.length > 0) {
        modified.push({ before: oldActivity, after: newActivity, changes });
      } else {
        unchanged.push(newActivity);
      }
    }
  }
